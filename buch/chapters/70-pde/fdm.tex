...
%\end{figure}
%Wir müssen jetzt die Wärmeleitungsgleichung diskretisieren.
...
%\]
%geschrieben werden.
%Wir kürzen den gemeinsamen Faktor $c=h_t\kappa/h_x^2$ ab.
%Ausserdem schreiben wir $u_k$ für den Spaltenvektor bestehend aus den
%Komponenten $u_{ik}$.
%
%Die Gleichung~\eqref{buch:pde:waerme:euler} gilt nur für innere Punkte,
%insbesondere erlaubt sie nicht, die Werte $u_{0k}$ und $u_{Nk}$ zu
%berechnen.
...
%$u_{N-1,k}$.
%Damit kann das Verfahren in Matrixform als
%\begin{equation}
%\begin{pmatrix}
%u_{0,k+1}\\
%u_{1,k+1}\\
...
%\end{figure}
%Dies ist natürlich das Euler-Verfahren.
%Abbildung~\ref{buch:pde:waerme:figure:euler} zeigt die Lösungsfläche.
%Die Lösungsvektoren $u_{k+1}$ entstehen durch Iteration der Matrix
%$A$ als $u_{k+1}=Au_k$.
%Die Iteration kann nur stabil bleiben, wenn der Spektralradius $\varrho(A)$
%von $A$ kleiner als $1$ ist.
%Experimentell kann man bestätigen, dass $\varrho(A)<1$ nur für $c<\frac12$ 
%eintritt, also wenn $h_t$ deutlich kleiner ist als $h_x$.
%Wir werden auf die Frage der Stabilität später nochmals zurückkommen.
%
...
%\]
%schreiben.
%Jetzt muss noch die Randbedingung berücksichtigt werden.
%Wegen $u_{0k}=u_{1k}$ kann man die Variable $u_{0k}$ aus
%der ersten Gleichung
%\begin{align*}
%-cu_{0k} + (1+2c)u_{1k} -cu_{2k}
%&=u_{1,k-1}
%\intertext{eliminieren und erhält}
%(1+c)u_{1k} -cu_{2k} &= u_{1,k-1}.
%\end{align*}
%Analog erhält man für die letzte Gleichung
%\begin{align*}
%-cu_{N-2,k} + (1+2c)u_{N-1,k} -cu_{N,k}
%&=u_{N-1,k-1}
%\\
%-cu_{N-2,k} + (1+c)u_{N-1,k}
%&=u_{N-1,k-1}.
%\end{align*}
%In Matrixform wird dies zu
%\begin{equation}
%Bu_k
%=
%\begin{pmatrix}
...
%u_{k-1}
%\end{equation}
%Die Lösung kann also durch Iteration von $u_k = B^{-1}u_{k-1}$ 
%erhalten werden, sie ist in Abbildung~\ref{buch:pde:waerme:figure:rueckwaerts}
%dargestellt..
%
%\begin{figure}
%\centering
