%
% laplace.tex -- template for standalon tikz images
%
...
\fill[color=red!10]
	({0.5*\s},{\s}) arc (0:180:{0.5*\s}) 
	--
	({-0.5*\s},{\s}) arc (0:-90:{0.5*\s})
	--
	({-\s},{0.5*\s}) arc (90:270:{0.5*\s})
	--
	({-\s},{-0.5*\s}) arc (90:0:{0.5*\s})
	--
	({-0.5*\s},{-\s}) arc (180:360:{0.5*\s})
	--
	({0.5*\s},{-\s}) arc (180:90:{0.5*\s})
	--
	({\s},{-0.5*\s}) arc (-90:90:{0.5*\s})
	--
	({\s},{0.5*\s}) arc (-90:-180:{0.5*\s})
	-- cycle;
\foreach \x in {-2,...,2}{
	\draw[line width=0.2pt]
		({\x*\s},{-2.2*\s}) -- ({\x*\s},{2.2*\s});
}
\node at ({-2*\s},{-2.2*\s}) [below] {$i-2$};
\node at ({-1*\s},{-2.2*\s}) [below] {$i-1$};
\node at (0,{-2.2*\s}) [below] {$i$};
\node at ({1*\s},{-2.2*\s}) [below] {$i+1$};
\node at ({2*\s},{-2.2*\s}) [below] {$i+2$};
\foreach \y in {-2,...,2}{
	\draw[line width=0.2pt]
...
\node at (0.1,{\s+0.1}) [above right] {$u_{i,k+1}$};
\node at (0.1,{-\s-0.1}) [below right] {$u_{i,k-1}$};
\node at ({-1.5*\s},{-2*\s}) [above] {$h_x$};
\node at ({-2*\s},{1.5*\s}) [right] {$h_y$};

\node at (-4.3,{-2*\s}) [right] {$\mathstrut k-2$};
\node at (-4.3,{-1*\s}) [right] {$\mathstrut k-1$};
\node at (-4.3,{0*\s}) [right] {$\mathstrut k$};
\node at (-4.3,{1*\s}) [right] {$\mathstrut k+1$};
\node at (-4.3,{2*\s}) [right] {$\mathstrut k+2$};
