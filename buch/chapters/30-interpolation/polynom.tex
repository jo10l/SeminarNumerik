...
\]
Geometrisch bedeutet dies, dass die ebene Kurve $t\mapsto (x(t),y(t))$
eine Tangen parallel zur Sekante durch $(x(a),y(a))$ und $(x(b),y(b))$ hat.
\end{satz}

\begin{proof}[Beweis]
Mit der Notation $\Delta x=x(b)-x(a)$ und $\Delta y=y(b)-y(a)$ betrachte
die Funktion
\[
f(t)
=
x(t) \Delta y - y(t) \Delta x
\]
hat die gleichen Werte
\begin{align*}
f(a)
&=
x(a) \bigl(y(b)-y(a)\bigr) - y(a)\bigl(x(b)-x(a)\bigr)
=
x(a)y(b)-y(a)x(b)
\\
f(b)
&=
x(b) \bigl(y(b)-y(a)\bigr) - y(b)\bigl(x(b)-x(a)\bigr)
=
-x(b)y(a)+y(b)x(a)
\end{align*}
an den Intervallenden.
Nach dem Satz von Rolle~\ref{buch:satz:rolle}
gibt es einen Zwischenwert $\xi$ derart, dass
\[
0
=
f'(\xi)
=
x'(\xi) \Delta y - y'(\xi)\Delta x
