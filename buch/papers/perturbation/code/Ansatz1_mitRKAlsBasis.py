...
for t in range(1,26):
    ...
    if (t==1):
        ...
        ny_x = (x_rk[t-1] - r0x - (t-1)*v0x)/(t-1)**2
        ny_y = (y_rk[t-1] - r0y - (t-1)*v0y + 0.5*g*(t-1)**2)/(t-1)**2
...
plot_nicely(x_naive, y_naive, 'b', "Ohne Luftwiderstand")
plot_nicely(x_rk, y_rk, 'g', "Mit Luftwiderstand, Runge Kutta")
plot_nicely(x_lin, y_lin, 'r', "Linear korrigiert")
...
plt.legend()
plt.grid()
plt.savefig("figs/Ansatz1_LineareKorrektur.png")
plt.savefig("figs/perturbation_fig1.eps")  ####Arbeit Fig 1.
plt.savefig("figs/perturbation_fig1.png")  ####Arbeit Fig 1.
plt.show()
...
for t in range(2,26):
    ...
    if (t==2):
        ...
        b_help = lambda t: (x_rk[t] - r0x - v0x*t ) / t**2
        b = np.array([b_help(t-1), b_help(t-2)])
        ny_x1, ny_x2 = np.linalg.inv(A) @ b
        
        #Gleichung 6
        b_help = lambda t: (y_rk[t] - r0y - v0y*t + 0.5*g*t**2 ) / t**2
        b = np.array([b_help(t-1), b_help(t-2)])
...
plt.figure()
plt.title("Quadratische Korrektur")
plot_nicely(x_naive, y_naive, 'b', "Ohne Luftwiderstand")
plot_nicely(x_rk, y_rk, 'g*', "Mit Luftwiderstand, Runge Kutta")
plot_nicely(x_qd, y_qd, 'r', "Quadratisch korrigiert")
...
plt.legend()
plt.grid()
plt.savefig("figs/Ansatz1_QuadratischeKorrektur.png")
plt.savefig("figs/perturbation_fig2.png")
plt.savefig("figs/perturbation_fig2.eps")
plt.show()
