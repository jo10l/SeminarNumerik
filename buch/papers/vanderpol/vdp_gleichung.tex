...
&= -R_0 \cdot i_0 \biggl(\frac{i}{i_0} - \frac{1}{3} \left(\frac{i}{i_0} \right)^3 \biggl).  
\end{align*}
...
\end{equation*}
Die Maschengleichung wird nun nach dem zweiten Kirchoffschen Gesetz:

\begin{equation}
u_{\text{in}}(t) = u_R(t) + u_C(t) + u_L(t).
\label{vanderpol:equations:kirchoff}
\end{equation}
Unter der Annahme, dass die Spannung $u_{\text{in}}$ eine über die Zeit unveränderliche Grösse ist, d.h. $u_{\text{in}}(t) = U_0$, die beiden Seiten der Gleichung \ref{vanderpol:equations:kirchoff} werden nach $t$ abgeleitet:

\begin{align}
\frac{dU_0}{dt} &= \frac{d}{dt}(u_R(t) + u_C(t) + u_L(t))\\
0 &= -R_0 \cdot i_0 \biggl(\frac{1}{i_0} \frac{di}{dt} - \frac{1}{3 \cdot i_0^3} \frac{d}{dt} (i(t)^3) \biggl) + \frac{1}{C} i(t) + L \frac{d^2i}{dt^2} \notag \\
0 &= -R_0 \cdot i_0 \biggl(\frac{1}{i_0} \frac{di}{dt} - \frac{1}{3 \cdot i_0^3} \cdot \left( 3 \frac{di}{dt} i(t)^2 \right) \biggl) + \frac{1}{C} i(t) + L \frac{d^2i}{dt^2} \notag \\
0 &= \frac{d^2i}{dt^2} - \frac{R_0}{L}  \frac{di}{dt} \cdot \biggl(1- \left(\frac{i(t)}{i_0}\right)^2 \biggl) + \frac{1}{LC} i(t).
\end{align}
Die verschiedenen Konstanten werden wie folgt definiert:
\begin{equation*}
\frac{R_0}{L}=\mu, \quad C=\frac{\mu}{R_0}, \quad i_0 = 1.
\end{equation*}
Abschliessend wird die Gleichung:
\begin{equation}
...
Wenn es versuchen wird, die gleiche Gleichung zu lösen, aber mit einem konstanten R Wert, erhalten wir eine Differentialgleichung dieses Typs:
\begin{equation}
...
\end{equation}
wobei $\omega_0$ die Resonanzfrequenz und $\zeta$ die Dämpfungskonstante sind. Die analytische Lösung lautet wie folgt:
\begin{equation}
i(t) = e^{-\zeta t} \cos(\omega_dt+\varphi_0).
\end{equation}
Das bedeutet, dass im Falle der Gl. \ref{vanderpol:equations:vdp_i}, d.h. die Van der Pol Gleichung eine Schwingung vorliegt, die bei grossen Werten von $i^2$ einen positiven Dämpfungswert hat, der ihre Amplitude verringert. Bei kleinen Werten von $i^2$ ist der Dämpfungswert negativ, was dazu führt, dass die Amplitude grösser wird und somit in diesen beiden Zuständen schwingt.
...
\begin{figure}
	...
	\caption{Phase und Zeit Raum Diagramm der Gl. \ref{vanderpol:equations:homogene_1} mit verschiedenen Werten von $\mu$\label{vanderpol:figures:homogene}}
\end{figure}
...
Ausgehend von der Formel der inohomogenen Differentialgleichung
\begin{equation*}
