...
Nach Einsetzen der Taylorreihen in das Eigenwertproblem (\refeq{ew:eq:eig}) erhalten wir den Ansatz
\begin{align}
...
Dies macht die Matrix auch selbstadjungiert, also $H_0 = H_0^T$. Dementsprechend ist der erste Term auf der linken seite
\begin{equation}
    \bm v_{0j}^T \bm H_0 \bm v_{1i}
    =
...
Die Eigenwerte $\lambda_{1i}$ können nun einfach in \refeq{ew:eq:eigval} eingesetzt werden und liefern bereits eine fertige Formel für das gesuchte
\begin{align*}
    \lambda_i(\varepsilon)
    &=
    \lambda_{0i} + \varepsilon \lambda_{1i} \\
...
Die hergeleitete Formel \refeq{ew:eq:explicit_eigvecs} für die Eigenvektoren ist nicht definiert für den folgenden Spezialfall:
...
\begin{align}
    ...
    = 0.
\end{align}
Falls der Zähler von \refeq{ew:eq:f2} null ist, geht die Gleichung auf.
