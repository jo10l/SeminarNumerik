...
Die Eigenwerte $\lambda_{1i}$ können einfach in \refeq{ew:eq:eigval} eingesetzt werden und liefern bereits eine fertige Formel für das gesuchte
\begin{align*}
    \lambda_i(\varepsilon)
    &=
    \lambda_{0i} + \varepsilon \lambda_{1i} \\
...
Der offensichtliche Fall ist das Multiplizieren mit $-1$ aber auch jede Komplexe zahl $e^(i*\gamma)$ ist möglich.
Der unbekannte Term $\bm v_{1i})$ ist also derjenige freiheitsgrad, welcher die Eigenwerte richtig skaliert.
Aus diesem Grund kann man definieren:
\begin{equation}
    ( 1 + (\bm v_{0i}^T \bm v_{1i}) ) = e^(i*\gamma)
\end{equation}
und für kleine $\varepsilon$, approximiert in erster Ordnung
\begin{equation}
